let italyHoldingsChart = null;
let italyPercentageChart = null;
...
function renderHoldingsChart(data) {
    const ctx = document.getElementById('italyHoldingsChart').getContext('2d');
    if (italyHoldingsChart) italyHoldingsChart.destroy();
    ...
    italyHoldingsChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
...
function renderPercentageChart(data) {
    const ctx = document.getElementById('italyPercentageChart').getContext('2d');
    if (italyPercentageChart) italyPercentageChart.destroy();
    ...
    italyPercentageChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
        options: chartOptions('Share of total foreign holdings in Italy (%)', true)
    });
}
...
async function fetchAndRenderItalyData() {
    try {
        const response = await fetch(`${API_BASE}/api/italy-treasury-dashboard`);
        if (!response.ok) throw new Error(`Server error ${response.status}`);
        const data = await response.json();

        const titleEl = document.getElementById('italySubtitleYear');
        ...
        console.error('Error loading Italy treasury dashboard data:', error);
        alert('Could not load Italy Treasury data. Please check API server and try again.');
    }
}

window.addEventListener('DOMContentLoaded', async () => {
    await checkServerStatus();
    await fetchAndRenderItalyData();

    if (typeof window.fetchEcbShssByIssuer === 'function') {
        await window.fetchEcbShssByIssuer({
            issuerCode: 'IT',
            startPeriod: '2021-Q1',
            canvasId: 'italyEcbFlowsChart',
            statusId: 'italyEcbStatus',
            logId: 'italyEcbLog'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsProductivity === 'function') {
        await window.loadTradingEconomicsProductivity({
            countrySlug: 'italy',
            statusId: 'italyProductivityStatus',
            imageId: 'italyProductivityImage',
            textId: 'italyProductivityText',
            linkId: 'italyProductivitySource'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsTopTaxRate === 'function') {
        await window.loadTradingEconomicsTopTaxRate({
            countrySlug: 'italy',
            statusId: 'italyTaxStatus',
            imageId: 'italyTaxImage',
            textId: 'italyTaxText',
            linkId: 'italyTaxSource',
            metricsId: 'italyTaxMetrics'
        });
    }
});
