let belgiumHoldingsChart = null;
let belgiumPercentageChart = null;
...
function renderHoldingsChart(data) {
    const ctx = document.getElementById('belgiumHoldingsChart').getContext('2d');
    if (belgiumHoldingsChart) belgiumHoldingsChart.destroy();
    ...
    belgiumHoldingsChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
...
function renderPercentageChart(data) {
    const ctx = document.getElementById('belgiumPercentageChart').getContext('2d');
    if (belgiumPercentageChart) belgiumPercentageChart.destroy();
    ...
    belgiumPercentageChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
        options: chartOptions('Share of total foreign holdings in Belgium (%)', true)
    });
}
...
async function fetchAndRenderBelgiumData() {
    try {
        const response = await fetch(`${API_BASE}/api/belgium-treasury-dashboard`);
        if (!response.ok) throw new Error(`Server error ${response.status}`);
        const data = await response.json();

        const titleEl = document.getElementById('belgiumSubtitleYear');
        ...
        console.error('Error loading Belgium treasury dashboard data:', error);
        alert('Could not load Belgium Treasury data. Please check API server and try again.');
    }
}

window.addEventListener('DOMContentLoaded', async () => {
    await checkServerStatus();
    await fetchAndRenderBelgiumData();

    if (typeof window.fetchEcbShssByIssuer === 'function') {
        await window.fetchEcbShssByIssuer({
            issuerCode: 'BE',
            startPeriod: '2021-Q1',
            canvasId: 'belgiumEcbFlowsChart',
            statusId: 'belgiumEcbStatus',
            logId: 'belgiumEcbLog'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsProductivity === 'function') {
        await window.loadTradingEconomicsProductivity({
            countrySlug: 'belgium',
            statusId: 'belgiumProductivityStatus',
            imageId: 'belgiumProductivityImage',
            textId: 'belgiumProductivityText',
            linkId: 'belgiumProductivitySource'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsTopTaxRate === 'function') {
        await window.loadTradingEconomicsTopTaxRate({
            countrySlug: 'belgium',
            statusId: 'belgiumTaxStatus',
            imageId: 'belgiumTaxImage',
            textId: 'belgiumTaxText',
            linkId: 'belgiumTaxSource',
            metricsId: 'belgiumTaxMetrics'
        });
    }
});
