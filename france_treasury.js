let franceHoldingsChart = null;
let francePercentageChart = null;
...
function renderHoldingsChart(data) {
    const ctx = document.getElementById('franceHoldingsChart').getContext('2d');
    if (franceHoldingsChart) franceHoldingsChart.destroy();
    ...
    franceHoldingsChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
...
function renderPercentageChart(data) {
    const ctx = document.getElementById('francePercentageChart').getContext('2d');
    if (francePercentageChart) francePercentageChart.destroy();
    ...
    francePercentageChart = new Chart(ctx, {
        type: 'line',
        data: { datasets },
        options: chartOptions('Share of total foreign holdings in France (%)', true)
    });
}
...
async function fetchAndRenderFranceData() {
    try {
        const response = await fetch(`${API_BASE}/api/france-treasury-dashboard`);
        if (!response.ok) throw new Error(`Server error ${response.status}`);
        const data = await response.json();

        const titleEl = document.getElementById('franceSubtitleYear');
        ...
        console.error('Error loading France treasury dashboard data:', error);
        alert('Could not load France Treasury data. Please check API server and try again.');
    }
}

window.addEventListener('DOMContentLoaded', async () => {
    await checkServerStatus();
    await fetchAndRenderFranceData();

    if (typeof window.fetchEcbShssByIssuer === 'function') {
        await window.fetchEcbShssByIssuer({
            issuerCode: 'FR',
            startPeriod: '2021-Q1',
            canvasId: 'franceEcbFlowsChart',
            statusId: 'franceEcbStatus',
            logId: 'franceEcbLog'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsProductivity === 'function') {
        await window.loadTradingEconomicsProductivity({
            countrySlug: 'france',
            statusId: 'franceProductivityStatus',
            imageId: 'franceProductivityImage',
            textId: 'franceProductivityText',
            linkId: 'franceProductivitySource'
        });
    }
    ...
    if (typeof window.loadTradingEconomicsTopTaxRate === 'function') {
        await window.loadTradingEconomicsTopTaxRate({
            countrySlug: 'france',
            statusId: 'franceTaxStatus',
            imageId: 'franceTaxImage',
            textId: 'franceTaxText',
            linkId: 'franceTaxSource',
            metricsId: 'franceTaxMetrics'
        });
    }
});
